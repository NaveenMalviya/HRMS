...
const Skill = () => {
    ...
    const Deletemulti = async () => {
        ...
        try {
            const response = await axios.delete(`${BASE_API_URL}skill/multidelete`, {
    ...
    const validateForm = () => {
        ...
        if (!formData.skills.trim()) {
            newErrors.skills = "This is a required field";
            isValid = false;
        }
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${BASE_API_URL}skill/list`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
            try {
                const response = await axios.post(`${BASE_API_URL}skill/createskill`, formData);
                settogle(!togle)
    ...
    const DeleteData = (id) => {
        ...
        if (isConfirmed) {
            ...
            try {
                console.log('id', id)
                const response = axios.delete(`${BASE_API_URL}skill/delete/?id=${id}`)
                                                ...
                                                <div class="signup-form">
                                                    ...
                                                    <form onSubmit={handleSubmit} class="mt-5 border p-4 bg-light shadow">
                                                        <div style={{ textAlign: 'center' }}>
                                                            <h4 style={{ display: 'inline', marginRight: '10px' }} className="mb-5 text-secondary">Create Skill</h4>
                                                        ...
                                                        <div class="row">
                                                            <div class="mb-3 col-md-6">
                                                                <input type="text" name="skills" value={formData.skills} onChange={handleInputChange} class="form-control" placeholder="Skills" />
                                                            ...
                                                            <div class="mb-3 col-md-6">
                                                                <input type="text" name="profile_id" value={formData.profile_id} onChange={handleInputChange} class="form-control" placeholder="Profile Id" />
                            ...
                            <tbody className="customtable">
                                {currentItems.map((data, index) => (
                                    <tr key={index}>
                                        ...
                                        <ModalBox isOpen={modalIsOpen} skillId={selectedSkillId} onRequestClose={closeModal}>
                                            <h2>Modal Title</h2>
                                            <p>Modal Content</p>
                                        </ModalBox>
                                    </tr>
